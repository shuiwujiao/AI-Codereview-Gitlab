...
class CodeReviewer(BaseReviewer):
    ...
    def review_code_simple(self, diff, diffs, file_content):
        ...
        messages = [
            {
                "role": "system",
                'content': '你是资深编程专家，仅针对【需审查的单个diff】进行分析，完全忽略所有其他代码内容（包括完整文件内容和全部git diff），仅指出重大代码问题（安全漏洞、逻辑错误、算法低效、重复代码等）。反馈需：1. 极致精简，直击要害；2. 建议具体且有洞见；3. 忽略类型提示、文档及注释问题；4. 用严谨Markdown格式（仅必要分级）5. 只给出问题和修改意见，不需要提供修改示例代码、总结；6. 中文回答，严格控制在200字内。回答模板参考： 1. **XX问题**\n   - **建议**: '
            },
            {
                "role": "user",
                "content": f"请专注review以下特定代码变更：\n【需审查的单个diff】：{diff}\n\n以下内容仅作为上下文内容参考，以便于理解代码逻辑和改动，无需review，也无需提出任何问题或意见：\n1. 完整文件内容：{file_content}\n2. 该文件的全部git diff：{diffs}\n\n请仅针对【需审查的单个diff】分析代码问题并提供修改意见，忽略所有参考内容中的代码细节。"
            },
        ]
        return self.call_llm(messages)

    def review_code_simple_v2(self, diffs, file_content):
        """
        去除了单个改动点的review，现在使用原本的 diffs + file_content 进行review
        """
        messages = [
            {
                "role": "system",
                'content': '你是资深编程专家，gitlab的commit代码变更将以git diff 字符串的形式提供，对应的完整文件内容也将一同提供。仅针对【需审查的diffs】进行分析，完全忽略完整文件内容中的其他代码内容，仅指出重大代码问题（安全漏洞、逻辑错误、算法低效、重复代码等）。反馈要求：1. 极致精简，直击要害；2. 建议具体且有洞见；3. 忽略类型提示、文档及注释问题；4. 用严谨Markdown格式（仅必要分级）5. 只给出问题和修改意见，不需要提供修改示例代码、总结；6. 中文回答，严格控制在200字内。回答模板参考： 1. **XX问题**\n   - **建议**: '
            },
            {
                "role": "user",
                "content": f"请专注review以下代码变更：\n【需审查的diffs】：{diffs}\n\n以下内容仅作为上下文内容参考，以便于理解代码逻辑和改动，无需review，也无需提出任何问题或意见：\n1. 完整文件内容：{file_content}\n\n请仅针对【需审查的diffs】分析代码问题并提供修改意见，忽略所有参考内容中的代码细节。"
            },
        ]
        return self.call_llm(messages)
